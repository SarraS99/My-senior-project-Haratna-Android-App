    ...
    public void onBindViewHolder(@NonNull MyHolder myHolder, final int i) {
        ...
        if (i==chatList.size()-1){
            if (chatList.get(i).isSeen()){
                myHolder.isSeenTv.setText("تم المشاهدة");
            }
            else {
                myHolder.isSeenTv.setText("تم الإرسال");
            }
        }
        else {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot ds: dataSnapshot.getChildren()){
                    ...
                    if (ds.child("sender").getValue().equals(myUID)){
                        ...
                        hashMap.put("message", "هذه الرسالة تم حذفها...");
                        ds.getRef().updateChildren(hashMap);

                        Toast.makeText(context, "تم حذف الرسالة...", Toast.LENGTH_SHORT).show();
                    }
                    else {
                        Toast.makeText(context, "يمكنك حذف رسائلك فقط...", Toast.LENGTH_SHORT).show();
                    }
