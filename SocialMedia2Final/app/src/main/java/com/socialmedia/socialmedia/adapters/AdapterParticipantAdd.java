...
public class AdapterParticipantAdd extends RecyclerView.Adapter<AdapterParticipantAdd.HolderParticipantAdd> {
    ...
    public void onBindViewHolder(@NonNull HolderParticipantAdd holder, int position) {
        ...
        try {
            Picasso.get().load(image).placeholder(R.drawable.user1_icon).into(holder.avatarIv);
        }
        catch (Exception e){
            holder.avatarIv.setImageResource(R.drawable.user1_icon);
        }
                                ...
                                if (dataSnapshot.exists()){
                                    ...
                                    if (myGroupRole.equals("creator")){
                                        if (hisPreviousRole.equals("admin")){
                                            ...
                                            options = new String[]{"حذف مشرف", "حذف مستخدم"};
                                            ...
                                            options = new String[]{"اجعله مشرف", "حذف مستخدم"};
                                            builder.setItems(options, new DialogInterface.OnClickListener() {
                                                @Override
                                                public void onClick(DialogInterface dialog, int which) {
                                                    //handle item clicks
                                        ...
                                        if (hisPreviousRole.equals("creator")){
                                            ...
                                            Toast.makeText(context, "أنشاء مجموعة...", Toast.LENGTH_SHORT).show();
                                        }
                                    ...
                                    AlertDialog.Builder builder = new AlertDialog.Builder(context);
                                    builder.setTitle("اضافة اعضاء")
                                            .setMessage("أضف هذا المستخدم في هذه المجموعة؟")
        ...
        ref.child(groupId).child("Participants").child(modelUser.getUid()).setValue(hashMap)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        //added successfully
                        Toast.makeText(context, "اضيف بنجاح...", Toast.LENGTH_SHORT).show();
                    }
                })
    ...
    private void makeAdmin(ModelUser modelUser) {
        ...
        reference.child(groupId).child("Participants").child(modelUser.getUid()).updateChildren(hashMap)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        //made admin
                        Toast.makeText(context, "المستخدم الآن مشرف ...", Toast.LENGTH_SHORT).show();
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
    ...
    private void removeAdmin(ModelUser modelUser) {
        ...
        reference.child(groupId).child("Participants").child(modelUser.getUid()).updateChildren(hashMap)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        //made admin
                        Toast.makeText(context, "لم يعد المستخدم مشرفًا ...", Toast.LENGTH_SHORT).show();
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
